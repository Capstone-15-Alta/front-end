...
const HeaderProfile = ({ data, getUserById }) => {
  ...
  // const [imageData, setImageData] = React.useState({loaded: false, data});
  // const fetchImage = React.useCallback(async () => {
  //   const url = data.image.slice(19);
  //   const result = await axiosClient.get(url);
  //   setImageData({loaded: true, data: result});
  // }, [data.image]);

  // React.useEffect(() => {
  //   if (data.image) fetchImage();
    ...
    <>
      <div className='headersss'>
        <div className='banner'>
          <div style={{ backgroundImage: `url(${data.image_cover})` }} className='banner-image'></div>
          {path === '/profile' || 'edit-profile' ? (
            <>
              ...
              />
              <input type='file' className='d-none' ref={bannerRef} onChange={handleUploadBanner} />
            </>
          ) : (
            ''
          )}
        ...
        <div className='profile-zzz'>
          {!!imageUrl ? (
            <Avatar className='foto' alt={data.username} src={axiosClient.get(imageUrl)} />
          ) : (
            <p>loading</p>
          )}
          {path === '/profile' || 'edit-profile' ? (
            <>
              ...
              />
              <input type='file' className='d-none' ref={imageRef} onChange={handleUploadImage} />
            </>
          ) : (
            ''
          )}
              ...
              {path === '/profile' ? (
                <div className='editBtn'>
                  <Link to='/edit-profile'>
                    <Button type='button' className='btn btnEditProfile' title='Edit Profile' />
                  </Link>
                </div>
              ) : path === '/edit-profile' ? (
                ''
                ...
                <div className='editBtn'>
                  {data.user_followers?.filter((is_follow) => is_follow.user_follower.id == userId).length > 0 ? (
                    <Button
                      type='button'
